...
public class SecurityConfig {
	...
	public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
		...
		http.authorizeHttpRequests(authroize -> authroize
				...
				.requestMatchers("/hopeboard/write", "/member/update", "/rentalReturn/rental", "/member/rentallist",
						"/freeboard/write")
				.authenticated()
				.requestMatchers("/member/list", "/book/register", "/rentalReturn/list", "/notice/write",
						"/wantbook/pagelist", "/event/write")
				.hasAnyAuthority("Admin")
				.requestMatchers("/member/**", "/hopeboard/**", "/book/**", "/rentalReturn/**", "/readingRoom/**",
						"/faq/**", "/freeboard/**", "/notice/**", "/board/**")
